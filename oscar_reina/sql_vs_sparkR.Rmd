...
sales_sdf %>%
  ...
  ggplot(aes(dt, items_sold)) + # works as well as the prevoius line
  geom_line(colour="darkorange") +
  theme_minimal()
```
...
# this is a sort of aggregate, adding the aggregated value to the full table
```{r}
"SELECT *
...
#### Query 2, left join, this is strictly correct if some shop id is missing, since the window function is ordering by shop id
```{r}
...
## Lateral view, splits by a field, expands vertically with as many resulting elements of the split
...
" %>%
  ...
  head
```
Use the window function to run the question from yesterday 
